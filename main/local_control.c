...
static int socket_tcp_client_create(const char *ip, uint16_t port)
{
    ...
    if (sockfd < 0) {
        ESP_LOGI(TAG, "socket create, sockfd: %d", sockfd);
        goto ERR_EXIT;
    }
    ...
    if (ret < 0) {
        ESP_LOGI(TAG, "socket connect, ret: %d, ip: %s, port: %d",
                 ret, ip, port);
        goto ERR_EXIT;
    }
    return sockfd;
...
void tcp_client_write_task(void *arg)
{
    ...
    while (1) {
        if (g_sockfd == -1) {
            ...
            ESP_LOGI(TAG, "TCP conn g_sockfd=%d", g_sockfd);
            continue;
        }
        ...
        ESP_LOGI(TAG, "TCP write, size: %d, data: %s", size, data);
        ret = write(g_sockfd, data, size);
        free(data);

        if (ret <= 0) {
            ESP_LOGI(TAG, "<%s> TCP write", strerror(errno));
            close(g_sockfd);
            g_sockfd = -1;
            continue;
        }
